...
if ($is_admin && !$token) {
    get_session("ss_delete_token", $token = uniqid(time()));
}
...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    if (!strstr($row['wr_option'], 'secret') ||
        ...
        if(!get_session($ss_name))
            $list[$i]['content'] = '
		<a href="./password.php?w=sc&amp;bo_table='.$bo_table.'&amp;wr_id='.$list[$i]['wr_id'].$qstr.'" class="hidden-xs btn btn-warning"  data-toggle="modal" data-target="#Modal" class="hidden-xs btn btn-default">댓글내용 확인</a>
		<a href="./password.php?w=sc&amp;bo_table='.$bo_table.'&amp;wr_id='.$list[$i]['wr_id'].$qstr.'" class="visible-xs btn btn-warning">댓글내용 확인</a>';
        else {
            $list[$i]['content'] = conv_content($row['wr_content'], 0, 'wr_content');
            $list[$i]['content'] = search_font($stx, $list[$i]['content']);
        }
