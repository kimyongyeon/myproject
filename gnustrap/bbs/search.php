...
if ($stx) {
    ...
    for ($i=0; $i<count($g5_search['tables']); $i++) {
        ...
        if ($row['cnt']) {
            ...
            $sch_class = "";
            $sch_all = "";
            if ($onetable == $g5_search['tables'][$i]) $sch_class = "class=active";
            else $sch_all = "class=active";
            $str_board_list .= '<li '.$sch_class.'><a href="'.$_SERVER['PHP_SELF'].'?'.$search_query.'&amp;gr_id='.$gr_id.'&amp;onetable='.$g5_search['tables'][$i].'" '.$sch_class.'>'.$row2['bo_subject'].' <span class="badge">'.$row['cnt'].'</span></a></li>';
        }
    }
