...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    $str = '';
    if ($cur_page > 1) {
        $str .= '<a href="'.$url.'1'.$add.'" class="tooltip-top" title="처음"><i class="fa fa-angle-double-left"></i></a>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) $str .= '<li><a href="'.$url.($start_page-1).$add.'" class="tooltip-top" title="이전"><i class="fa fa-angle-left"></i></a></li>'.PHP_EOL;
    ...
    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                ...
                $str .= '<li class="active"><a href="#" class="tooltip-top" title="현재페이지">'.$k.'</a></li>'.PHP_EOL;
        }
    }

    if ($total_page > $end_page) $str .= '<li><a href="'.$url.($end_page+1).$add.'" class="tooltip-top" title="다음"><i class="fa fa-angle-right"></i></a></li>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<li><a href="'.$url.$total_page.$add.'" class="tooltip-top" title="맨끝"><i class="fa fa-angle-double-right"></i></a></li>'.PHP_EOL;
    }

    if ($str)
        return "<div class=\"panel-footer\"><nav class=\"text-center\"><ul class=\"pagination\"><li>{$str}</li></ul></nav></div>";
    else
        return "";
}
...
{
    if(!$paging_html)
        $paging_html = '<div class=\"panel-footer\"><nav class=\"text-center\"><ul class="pagination"><li></li></ul></nav></div>';

    return preg_replace("/^(<ul[^>]+><li[^>]+>)/", '$1'.$insert_html.PHP_EOL, $paging_html);
}
...
{
    if(!$paging_html)
        $paging_html = '<ul class="pagination"><li></li></ul>';

    if(preg_match("#".PHP_EOL."</li></ul>#", $paging_html))
        $php_eol = '';
    else
        $php_eol = PHP_EOL;

    return preg_replace("#(</li></ul>)$#", $php_eol.$insert_html.'$1', $paging_html);
}
...
function get_list($write_row, $board, $skin_url, $subject_len=40)
{
    ...
    $list['comment_cnt'] = '';
    if ($list['wr_comment'])
        $list['comment_cnt'] = "<span class=\"tooltip-top\" title=\"댓글수\" style=\"color: #F00; font-size: 11px;\">[".$list['wr_comment']."]</span>";
    ...
    if ($board['bo_use_sideview'])
        ...
        $list['name'] = '<span class="'.($list['mb_id']?'sv_member':'sv_guest').' tooltip-top" title="'.$list['mb_id'].'">'.$tmp_name.'</span>';
    ...
    $list['icon_reply'] = '';
    if ($list['reply'])
        $list['icon_reply'] = '<i class="fa fa-mail-reply fa-fw fa-rotate-180 tooltip-top" title="답변글" style="margin-left:'.$list['reply'].'px;"></i>';

    $list['icon_link'] = '';
    if ($list['wr_link1'] || $list['wr_link2'])
        $list['icon_link'] = '<i class="fa fa-external-link fa-fw tooltip-top" title="관련링크"></i>';
    ...
    if ($board['bo_new'] && $list['wr_datetime'] >= date("Y-m-d H:i:s", G5_SERVER_TIME - ($board['bo_new'] * 3600)))
        $list['icon_new'] = '<i class="fa fa-pencil fa-fw tooltip-top" title="새글"></i>';

    $list['icon_hot'] = '';
    if ($board['bo_hot'] && $list['wr_hit'] >= $board['bo_hot'])
        $list['icon_hot'] = '<i class="fa fa-heart-o fa-fw tooltip-top animation-pulse" title="인기글"></i>';

    $list['icon_secret'] = '';
    if (strstr($list['wr_option'], 'secret'))
        $list['icon_secret'] = '<i class="fa fa-lock fa-fw tooltip-top" title="비밀글"></i>';
    ...
    if ($list['file']['count'])
        $list['icon_file'] = '<i class="fa fa-save fa-fw tooltip-top" title="파일첨부"></i>';
...
function get_sideview($mb_id, $name='', $email='', $homepage='')
{
    ...
    if($g5['sms5_use_sideview']){
        ...
        if( $mb['mb_open'] && $mb['mb_sms'] && $mb['mb_hp'] ) $str2 .= "<li><a href=\"".G5_SMS5_URL."/?mb_id=".$mb_id."\" class=\"win_sms5\" target=\"_blank\"><i class=\"fa fa-paper-plane-o fa-fw\"></i> 문자보내기</a></li>\n";
    }
...
function check_url_host($url, $msg='', $return_url=G5_URL)
{
    ...
    if ((isset($p['scheme']) && $p['scheme']) || (isset($p['host']) && $p['host'])) {
		...
		if ($p['host'] != $host) {
            ...
            echo '<p><a href="'.$return_url.'" class="btn btn-default"><i class="fa fa-undo fa-fw"></i> 돌아가기</a></p>'.PHP_EOL;
            echo '</noscript>'.PHP_EOL;
            exit;
        }
    }
