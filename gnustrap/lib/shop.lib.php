    ...
    if($subj_count > 1) {
        ...
        for($i=0; $i<$subj_count; $i++) {
            ...
            if($opt_count) {
                ...
                $select = '<select id="it_option_'.$seq.'" class="it_option form-control"'.$disabled.'>'.PHP_EOL;
        ...
        $select = '<select id="it_option_1" class="it_option form-control">'.PHP_EOL;
        $select .= '<option value="">선택</option>'.PHP_EOL;
        for($i=0; $row=sql_fetch_array($result); $i++) {
            if($row['io_price'] >= 0)
...
function get_item_supply($it_id, $subject)
{
    ...
    for($i=0; $i<$subj_count; $i++) {
        ...
        if($opt_count) {
            ...
            $select = '<select id="it_supply_'.$seq.'" class="it_supply form-control">'.PHP_EOL;
            $select .= '<option value="">선택</option>'.PHP_EOL;
            for($k=0; $k<$opt_count; $k++) {
                $opt_val = $opt[$k];
                if($opt_val) {
...
function relation_item($it_id, $width, $height, $rows=3)
{
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        if($i == 0) {
            ...
            $str .= '<ul class="sct_rel_ul list-inline">';
        }
...
function get_sns_share_link($sns, $url, $title, $img)
{
    ...
    switch($sns) {
        case 'facebook':
            $str = '<a href="https://www.facebook.com/sharer/sharer.php?u='.urlencode($url).'&amp;p='.urlencode($title).'" target="_blank" class="btn btn-default popover-top" data-container="body" data-toggle="popover-top" data-content="페이스북으로 보내기를 요청합니다." data-original-title="페이스북"><i class="fa fa-facebook"></i></a>';
            break;
        case 'twitter':
            $str = '<a href="https://twitter.com/share?url='.urlencode($url).'&amp;text='.urlencode($title).'" target="_blank" class="btn btn-default popover-top" data-container="body" data-toggle="popover-top" data-content="트위터로 보내기를 요청합니다." data-original-title="트위터"><i class="fa fa-twitter"></i></a>';
            break;
        case 'googleplus':
            $str = '<a href="https://plus.google.com/share?url='.urlencode($url).'" target="_blank" class="btn btn-default popover-top" data-container="body" data-toggle="popover-top" data-content="구글플러스로 보내기를 요청합니다." data-original-title="구글플러스"><i class="fa fa-google-plus"></i></a>';
            break;
        case 'kakaotalk':
            if($config['cf_kakao_js_apikey'])
                $str = '<a href="javascript:kakaolink_send(\''.str_replace('+', ' ', urlencode($title)).'\', \''.urlencode($url).'\');" class="btn btn-default popover-top" data-container="body" data-toggle="popover-top" data-content="카카오톡으로 보내기를 요청합니다." data-original-title="카카오톡"><img src="'.$img.'" alt="카카오톡 링크보내기"></a>';
            break;
    }
