...
<div class="well">
    <?php if ($page > 1) {?><a href="<?php echo $_SERVER['PHP_SELF']; ?>" class="btn btn-default">처음으로</a><?php } ?>
	<span>전체 FAQ <?php echo $total_count; ?>건</span>
</div>

<div class="well well-sm">
        FAQ는 무제한으로 등록할 수 있습니다<br />
        <strong>FAQ추가</strong>를 눌러 FAQ Master를 생성합니다. (하나의 FAQ 타이틀 생성 : 자주하시는 질문, 이용안내..등 )<br />
        생성한 FAQ Master 의 <strong>제목</strong>을 눌러 세부 내용을 관리할 수 있습니다.
</div>

<div>
    <a href="./faqmasterform.php" class="btn btn-primary"><i class="fa fa-plus-circle fa-fw"></i> FAQ추가</a>
</div>
...
<div class="table-responsive">
    ...
    <tr>
        ...
        <td>
            <a href="./faqmasterform.php?w=u&amp;fm_id=<?php echo $row['fm_id']; ?>" class="btn btn-default"><span class="sound_only"><?php echo stripslashes($row['fm_subject']); ?> </span><i class="fa fa-file-text-o fa-fw"></i> 수정</a>
            <a href="<?php echo G5_BBS_URL; ?>/faq.php?fm_id=<?php echo $row['fm_id']; ?>" class="btn btn-default"><span class="sound_only"><?php echo stripslashes($row['fm_subject']); ?> </span><i class="fa fa-eye fa-fw"></i> 보기</a>
            <a href="./faqmasterformupdate.php?w=d&amp;fm_id=<?php echo $row['fm_id']; ?>" onclick="return delete_confirm();" class="btn btn-default"><span class="sound_only"><?php echo stripslashes($row['fm_subject']); ?> </span><i class="fa fa-cut fa-fw"></i> 삭제</a>
        </td>
    </tr>
    <?php
    }
    ...
    if ($i == 0){
        echo '<tr><td colspan="5" class="text-center"><span>자료가 한건도 없습니다.</span></td></tr>';
    }
    ?>
    </tbody>
    </table>
