...
?>
<div class="row">
    <div id="send_emo" class="col-md-6">
        <h4><span class="label label-info"><i class="fa fa-github"></i> 이모티콘 목록</span></h4>
        <?php include_once('./sms_write_form.php'); ?>
    </div>

    <div id="send_write" class="col-md-6">
        ...
        <h4><span class="label label-info"><i class="fa fa-send-o"></i> 보낼내용</span></h4>
        <div class="sms5_box write_wrap">
            <span class="box_ico"></span>
            <label for="wr_message" id="wr_message_lbl">내용</label>
            <textarea name="wr_message" id="wr_message" class="box_txt box_square textarea form-control" onkeyup="byte_check('wr_message', 'sms_bytes');" accesskey="m"></textarea>

            <div id="sms_byte"><span id="sms_bytes">0</span> / 80 byte</div>

            <button type="button" id="write_sc_btn" class="btn btn-default write_scemo_btn">특수<br>기호</button>
            ...
            <button type="button" id="write_emo_btn" class="btn btn-default write_scemo_btn">이모<br>티콘</button>
        ...
        <div id="write_preset">
            <h4><span class="label label-info">{이름} : 받는사람 이름</span></h4>
        </div>

        <div id="write_reply">
            <label for="wr_reply">회신<strong class="sound_only"> 필수</strong></label>
            <input type="text" name="wr_reply" value="<?php echo $sms5['cf_phone']?>" id="wr_reply" required class="form-control required" size="17" maxlength="20">
        </div>
		</div>
		<div class="clearfix"></div>
		<hr>

        <div id="write_recv" class="write_inner col-md-6">
            <h4><span class="label label-info pull-left"><i class="fa fa-inbox"></i> 받는사람</span></h4>
            <button type="button" class="btn btn-default pull-right" onclick="hp_list_del()" style="margin-top:-15px; margin-bottom:15px;">선택삭제</button>

            <label for="hp_list" class="sound_only">받는사람들</label>
            <select name="hp_list" id="hp_list" size="5" class="form-control"></select>

            <div id="recv_add" class="row">
                <div class="col-md-4" style="margin:15px 0;">
                <input type="text" name="hp_name" id="hp_name" class="form-control" size="11" maxlength="20" onkeypress="if(event.keyCode==13) document.getElementById('hp_number').focus();" placeholder="이름">
				</div>
                <div class="col-md-4" style="margin:15px 0;">
                <input type="text" name="hp_number" id="hp_number" class="form-control" size="11" maxlength="20" onkeypress="if(event.keyCode==13) hp_add()" placeholder="번호">
				</div>
                <div class="col-md-4" style="margin:15px 0;">
				<button type="button" onclick="hp_add()" class="btn btn-primary">추가</button>
				</div>
            </div>
        </div>
        ...
        <div id="write_rsv" class="write_inner">
            <h4><span class="label label-info"><i class="fa fa-clock-o"></i> 예약전송</span></h4>

            <div class="write_floater">
                <label class="btn btn-default" for="wr_booking"><span class="sound_only">예약전송 </span>사용
                <input type="checkbox" name="wr_booking" id="wr_booking" onclick="booking(this.checked)" data-toggle="checkbox">
				</label>
            </div>
            ...
            <div class="row">
			  ...
			  <div class="input-group">
                <select name="wr_by" id="wr_by" disabled class="form-control">
                <option value="<?php echo date('Y')?>"><?php echo date('Y')?></option>
                <option value="<?php echo date('Y')+1?>"><?php echo date('Y')+1?></option>
                </select>
                <span class="input-group-addon"><label for="wr_by">년</label></span>
              </div>
			  </div>
              <div class="col-md-4" style="margin:15px 0;">
			  <div class="input-group">
                <select name="wr_bm" id="wr_bm" disabled class="form-control">
                <?php for ($i=1; $i<=12; $i++) { ?>
                <option value="<?php echo sprintf("%02d",$i)?>"<?php echo get_selected(date('m'), $i); ?>><?php echo sprintf("%02d",$i)?></option>
                <?php } ?>
                </select>
                <span class="input-group-addon"><label for="wr_bm">월</label></span>
              </div>
			  </div>
              <div class="col-md-4" style="margin:15px 0;">
			  <div class="input-group">
                <select name="wr_bd" id="wr_bd" disabled class="form-control">
                <?php for ($i=1; $i<=31; $i++) { ?>
                <option value="<?php echo sprintf("%02d",$i)?>"<?php echo get_selected(date('d'), $i); ?>><?php echo sprintf("%02d",$i)?></option>
                <?php } ?>
                </select>
                <span class="input-group-addon"><label for="wr_bd">일</label><br></span>
              </div>
			  </div>
            </div>
            ...
            <div class="row">
			  ...
			  <div class="input-group">
                <select name="wr_bh" id="wr_bh" disabled class="form-control">
                <?php for ($i=0; $i<24; $i++) { ?>
                <option value="<?php echo sprintf("%02d",$i)?>"<?php echo get_selected(date('H')+1, $i); ?>><?php echo sprintf("%02d",$i)?></option>
                <?php } ?>
                </select>
                <span class="input-group-addon"><label for="wr_bh">시</label></span>
              </div>
			  </div>
              <div class="col-md-6" style="margin:15px 0;">
			  <div class="input-group">
                <select name="wr_bi" id="wr_bi" disabled class="form-control">
                <?php for ($i=0; $i<=59; $i+=5) { ?>
                <option value="<?php echo sprintf("%02d",$i)?>"><?php echo sprintf("%02d",$i)?></option>
                <?php } ?>
                </select>
                <span class="input-group-addon"><label for="wr_bi">분</label></span>
              </div>
			  </div>
            </div>
...
<div id="book_tab" class="btn-group">
    <a href="#book_group" id="book_group" class="btn btn-primary">그룹</a>
    <a href="#book_person" id="book_person" class="btn btn-primary">개인</a>
    <a href="#book_level" id="book_level" class="btn btn-primary">권한</a>
</div>
		<hr >

        <div id="num_book"></div>

        <div class="well well-sm">SMS 수신을 허용한 회원님만 출력됩니다.</div>
    </div>
</div>
...
<section>
    <div class="alert alert-warning">SMS 문자전송 서비스를 사용할 수 없습니다.</div>
    <div class="well well-sm">
            SMS 를 사용하지 않고 있기 때문에, 문자 전송을 할 수 없습니다.<br>
            SMS 사용 설정은 <a href="../config_form.php#anc_cf_sms" class="label label-info"><i class="fa fa-link"></i> 환경설정 &gt; 기본환경설정 &gt; SMS설정</a> 에서 SMS 사용을 아이코드로 변경해 주셔야 사용하실수 있습니다.
    </div>
</section>
