...
<div class="table-responsive">
    ...
    <tr>
        ...
        <td><button type="button" class="form-control" onclick="sms_obj.person_add(<?php echo $res['bk_no']?>, '<?php echo get_text($res['bk_name']) ?>', '<?php echo $res['bk_hp']?>')">추가</button></td>
     </tr>
    <?php } ?>
    </tbody>
    </table>
...
<div class="center">
    <button type="button" class="btn btn-default" onclick="sms_obj.person_multi_add()">선택추가</button>
    <button type="button" class="btn btn-default" onclick="sms_obj.triggerclick('#book_group')" >그룹목록</button>
</div>
...
<form name="search_form" id="sms_person_form" method="get" class="form-inline" action="<?php echo $_SERVER['PHP_SELF']?>">
<input type="hidden" name="total_pg" value="<?php echo $total_page?>">
<input type="hidden" name="page" value="<?php echo $page?>">

<div class="form-group">
<label for="bg_no" class="sound_only">그룹</label>
<select name="bg_no" id="bg_no" class="form-control">
...
<select name="st" id="stt" class="form-control">
...
<input type="text" size="15" name="sv" value="<?php echo $sv?>" id="svv" required class="required form-control">
