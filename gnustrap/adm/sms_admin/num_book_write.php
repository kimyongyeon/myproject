...
<table>
    ...
    <tbody>
    <tr>
        <td><label for="bg_no">그룹 <strong class="sound_only"> 필수</strong></label>
            <select name="bg_no" id="bg_no" required class="required form-control">
    ...
    <tr>
        <td><label for="bk_name">이름<strong class="sound_only"> 필수</strong></label>
		<input type="text" name="bk_name" id="bk_name" maxlength="50" value="<?php echo $write['bk_name']?>" required class="form-control required"></td>
    </tr>
    <tr>
        <td><label for="bk_hp">휴대폰번호<strong class="sound_only"> 필수</strong></label>
            <input type="text" name="bk_hp" id="bk_hp" value="<?php echo $write['bk_hp']?>" required class="form-control required">
    ...
    <?php if ($w == 'u') { ?>
    <tr>
        <td><strong>회원아이디</strong><br /> <?php echo $write['mb_id'] ? '<a href="'.G5_ADMIN_URL.'/member_form.php?w=u&amp;mb_id='.$write['mb_id'].'">'.$write['mb_id'].'</a>' : '비회원'?> </td>
    </tr>
    <tr>
        <td><strong>업데이트</strong><br /> <?php echo $write['bk_datetime']?> </td>
    </tr>
    <?php } ?>
    <tr>
        <td><label for="bk_memo">메모</label>
            <textarea name="bk_memo" id="bk_memo" class="textarea form-control"><?php echo $write['bk_memo']?></textarea>
        </td>
    </tr>
    </tbody>
</table>
...
<div class="center">
    <input type="submit" value="확인" class="btn btn-default" accesskey="s" onclick="return book_submit();">
    <a href="./num_book.php?<?php echo $_SERVER['QUERY_STRING']?>" class="btn btn-default">목록</a>
</div>
