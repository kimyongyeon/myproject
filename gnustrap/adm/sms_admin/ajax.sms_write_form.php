...
while($res = sql_fetch_array($qry)) 
{
    ...
    <li class=\"screen_list sms5_box\">
        ...
        <textarea readonly class=\"form-control textarea sms_textarea box_txt box_square\" onclick=\"emoticon_list.go({$res['fo_no']})\">{$res['fo_content']}</textarea>
        <textarea id=\"fo_contents_{$res['fo_no']}\" class=\"form-control textarea\" style=\"display:none\">{$res['fo_content']}</textarea>
        <strong class=\"label label-default\">".cut_str($res['fo_name'],20)."</strong>
    </li>
    ";
}
