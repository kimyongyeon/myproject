...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tbody>
    <tr>
        <td><label for="co_id">ID</label>
            <?php echo help('20자 이내의 영문자, 숫자, _ 만 가능합니다.'); ?>
            <input type="text" value="<?php echo $co['co_id']; ?>" name="co_id" id ="co_id" required <?php echo $readonly; ?> class="required <?php echo $readonly; ?> form-control" size="20" maxlength="20">
            <?php if ($w == 'u') { ?><a href="<?php echo G5_BBS_URL; ?>/content.php?co_id=<?php echo $co_id; ?>" class="btn btn-default" style="margin-top:5px"><i class="fa fa-reply"></i> 내용확인</a><?php } ?>
        </td>
    </tr>
    <tr>
        <td><label for="co_subject">제목</label>
		<input type="text" name="co_subject" value="<?php echo htmlspecialchars2($co['co_subject']); ?>" id="co_subject" required class="form-control required" size="90"></td>
    </tr>
    <tr>
        <td><strong>내용</strong>
    ...
    <tr>
        <td><label for="co_include_head">상단 파일 경로</label>
            <?php echo help("설정값이 없으면 기본 상단 파일을 사용합니다."); ?>
            <input type="text" name="co_include_head" value="<?php echo $co['co_include_head']; ?>" id="co_include_head" class="form-control" size="60">
        </td>
    </tr>
    <tr>
        <td><label for="co_include_tail">하단 파일 경로</label>
            <?php echo help("설정값이 없으면 기본 하단 파일을 사용합니다."); ?>
            <input type="text" name="co_include_tail" value="<?php echo $co['co_include_tail']; ?>" id="co_include_tail" class="form-control" size="60">
        </td>
    </tr>
    <tr>
        <td><label for="co_himg">상단이미지</label>
            <input type="file" name="co_himg" id="co_himg" class="form-control">
            <?php
    ...
    <tr>
        <td><label for="co_timg">하단이미지</label>
            <input type="file" name="co_timg" id="co_timg" class="form-control">
            <?php
